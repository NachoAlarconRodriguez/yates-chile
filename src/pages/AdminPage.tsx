...
export const AdminPage: React.FC<AdminPageProps> = ({ onNavigate }) => {
  ...
  const [editPolicies, setEditPolicies] = useState<ExpeditionPolicySection[]>(DEFAULT_EXPEDITION_POLICIES);
  ...
  const handleOpenEditDeparture = useCallback((dep: any) => {
    ...
    let initialPolicies: ExpeditionPolicySection[] = DEFAULT_EXPEDITION_POLICIES.map(p => ({ ...p }));
    if (dep.policies) {
      if (Array.isArray(dep.policies) && dep.policies.length > 0) {
        initialPolicies = dep.policies.map((p: any) => ({
          id: p.id || `pol_${Math.random().toString(36).substring(2, 9)}`,
          title: p.title || '',
          content: p.content || (Array.isArray(p.points) ? p.points.join('\n') : String(p.content || '')),
        }));
      } else if (typeof dep.policies === 'string') {
        try {
          const parsed = JSON.parse(dep.policies);
          if (Array.isArray(parsed) && parsed.length > 0) {
            initialPolicies = parsed.map((p: any) => ({
              id: p.id || `pol_${Math.random().toString(36).substring(2, 9)}`,
              title: p.title || '',
              content: p.content || (Array.isArray(p.points) ? p.points.join('\n') : String(p.content || '')),
            }));
    ...
    setEditPolicies(initialPolicies);

    const effectiveAvail = (dep.available_slots !== undefined && dep.available_slots !== null && dep.available_slots !== '')
      ? Math.max(0, Number(dep.available_slots))
      : (dep.total_slots ?? 6);

    let initialPolicyUrl = dep.policyUrl || (dep as any).policy_url || '';
    if (!initialPolicyUrl && typeof window !== 'undefined' && dep.id) {
      try {
        const cached = localStorage.getItem('yates_public_expeditions_cache');
        if (cached) {
          const list = JSON.parse(cached);
          const found = list.find((item: any) => item.id === dep.id);
          if (found && (found.policyUrl || found.policy_url)) {
            initialPolicyUrl = found.policyUrl || found.policy_url;
          }
        }
      } catch {}
      if (!initialPolicyUrl) {
...
${cust.notes || 'Sin notas adicionales.'}`;
  ...
  const handlePolicyPdfUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.success && res.url) {
        ...
        triggerAlert('Documento PDF de políticas cargado exitosamente. Haz clic en "Guardar Cambios" para confirmar.', 'info', 'PDF Conectado');
      } else {
        triggerAlert(res.error || 'Error al subir el archivo PDF.', 'error');
      }
    } catch (err: any) {
  ...
  const handleSubmitExpPassenger = async (e?: React.FormEvent) => {
    ...
    const rawAvail = typeof selectedExpeditionForPassenger.availablePax === 'number'
      ? selectedExpeditionForPassenger.availablePax
      : typeof selectedExpeditionForPassenger.spotsLeft === 'number'
      ? selectedExpeditionForPassenger.spotsLeft
      : undefined;
    if (rawAvail !== undefined && rawAvail <= 0) {
      triggerAlert('Esta expedición se encuentra completa (0 cupos disponibles). No es posible sumar más pasajeros.', 'warning');
      return;
    }
            ...
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="bg-white border border-slate-200/80 rounded-3xl p-5 shadow-[0_4px_24px_rgba(11,25,44,0.03)] space-y-3">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  {/* Grupo de Controles: Filtros Dropdown de Categoría + Estado + Vistas */}
                  <div className="flex flex-wrap items-center gap-2.5">
                    
                    {/* DROPDOWN FILTRO TIPO DE SERVICIO */}
                    <div className="relative">
                      <button
                        type="button"
                        onClick={() => {
                        ...
                        className={`h-8.5 px-3.5 rounded-full text-xs font-semibold transition cursor-pointer flex items-center gap-2 border shadow-2xs active:scale-95 select-none ${
                          bookingsTypeFilter !== 'all'
                            ? 'bg-[#0b192c] text-white border-[#0b192c]'
                            : isTypeFilterOpen
                            ? 'bg-slate-100 text-[#0b192c] border-slate-300'
                            : 'bg-[#0b192c] text-white border-[#0b192c]'
                        }`}
                      ...
                      >
                        ...
                        {bookingsTypeFilter === 'lodge' && (
                          <div className="flex items-center gap-1.5">
                            ...
                            <span>Tipo: Lodge ({allUnifiedBookings.filter((b) => b.type === 'lodge').length})</span>
                          </div>
                        )}
                        {bookingsTypeFilter === 'expedition' && (
                          <div className="flex items-center gap-1.5">
                            <Ship className="w-3.5 h-3.5 text-sky-300" />
                            <span>Tipo: Expediciones ({allUnifiedBookings.filter((b) => b.type === 'expedition').length})</span>
                          </div>
                        )}
                        ...
                        <>
                          ...
                          <div className="absolute left-0 top-full mt-1.5 z-40 w-52 bg-white border border-slate-200/90 rounded-2xl shadow-[0_12px_30px_rgba(11,25,44,0.15)] p-1.5 space-y-0.5 animate-fadeIn">
                            ...
                            >
                              ...
                              >
                                {allUnifiedBookings.filter((b) => b.type === 'lodge').length}
                              </span>
                            </button>
                            ...
                            >
                              ...
                              >
                                {allUnifiedBookings.filter((b) => b.type === 'expedition').length}
                              </span>
                            </button>
                    ...
                    <div className="relative">
                      <button
                        ...
                        title="Filtrar por estado de reserva"
                      >
                        {bookingsStatusFilter === 'all' && <span>Estado: Todos</span>}
                        {(bookingsStatusFilter === 'confirmed' || bookingsStatusFilter === 'approved') && (
                          <span>Estado: Confirmadas</span>
                        )}
                        {bookingsStatusFilter === 'reserved' && (
                          <span>Estado: Reservadas (50%)</span>
                        )}
                        {(bookingsStatusFilter === 'scheduled' || bookingsStatusFilter === 'pending_transfer') && (
                          <span>Estado: Agendadas (0%)</span>
                        )}
                        {bookingsStatusFilter === 'blocked' && (
                          <span>Estado: Bloqueos</span>
                        )}
                        <ChevronDown
                          className={`w-3.5 h-3.5 transition-transform duration-200 ${
                            isStatusFilterOpen ? 'rotate-180 text-sky-300' : 'text-slate-300'
                          }`}
                        />
                      </button>
                    ...
                    {/* Vista (Lista / Cards) */}
                    <div className="inline-flex items-center bg-slate-100 p-0.5 rounded-full border border-slate-200/60 shadow-2xs h-8.5">
                      <button
                        type="button"
                        onClick={() => setBookingsViewMode('list')}
                        className={`h-7.5 px-2.5 rounded-full transition cursor-pointer flex items-center justify-center ${
                                    ...
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        handleOpenEditDeparture({
                                          ...
                                          brochureUrl: (exp as any).brochureUrl || (exp as any).brochure_url || '',
                                          brochure_url: (exp as any).brochureUrl || (exp as any).brochure_url || '',
                                          policyUrl: (exp as any).policyUrl || (exp as any).policy_url || '',
                                          policy_url: (exp as any).policyUrl || (exp as any).policy_url || '',
                                          created_at: new Date().toISOString(),
                                        });
                                      }}
                                      className="w-7 h-7 rounded-full text-slate-400 hover:text-[#0b192c] hover:bg-slate-100 flex items-center justify-center transition cursor-pointer"
            ...
            <div className="bg-white rounded-3xl max-w-5xl w-full max-h-[92vh] flex flex-col shadow-[0_25px_60px_rgba(11,25,44,0.25)] border border-slate-200/90 overflow-hidden my-auto animate-scaleIn">
              ...
              <div className="px-6 py-3.5 bg-white border-b border-slate-100 flex flex-wrap items-center justify-between gap-3 shrink-0">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {(!exp.isSoldOut && exp.availablePax > 0) && (
                    <button
                      type="button"
                      onClick={() => {
              ...
              <div className="flex-1 flex flex-col justify-between bg-[#fbfcfd] overflow-hidden">
                ...
                <div className="px-6 sm:px-8 py-4 bg-white border-t border-slate-200/80 flex items-center justify-between gap-3 shrink-0">
                  ...
                  {expPassengerStep < 4 ? (
                    <button
                      type="button"
                      disabled={expPassengerStep === 1 && availableSlots <= 0}
                      onClick={() => {
            ...
            <div className="px-6 pt-3.5 pb-2 bg-slate-50/80 border-b border-slate-200/80">
              <div className="grid grid-cols-2 sm:grid-cols-4 p-1 bg-slate-200/70 rounded-2xl gap-1 text-xs font-semibold">
                ...
                >
                  ...
                  <span className="truncate">4. Brochure & Políticas</span>
                </button>
              </div>
            </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        required
                        value={editingDeparture.available_slots === 0 ? 0 : (editingDeparture.available_slots ?? 0)}
                        onChange={(e) => {
                          const val = e.target.value;
                          const nextAvail = val === '' ? 0 : Math.max(0, parseInt(val, 10) || 0);
                          setEditingDeparture({
                            ...editingDeparture,
              ...
              {editDepartureTab === 'brochure' && (() => {
                ...
                return (
                  <div className="space-y-4 animate-fadeIn">
                    ...
                    {/* Separator to Policies */}
                    <div className="pt-6 border-t border-slate-200" />

                    {/* Section 2: Políticas y Condiciones en PDF */}
                    <div className="space-y-4">
                      {/* Header Info Banner for Policies */}
                      <div className="bg-gradient-to-r from-[#0b192c] to-[#1e3a5f] p-4 rounded-2xl text-white flex items-start gap-3.5 shadow-sm border border-white/10">
                        <div className="w-10 h-10 rounded-xl bg-amber-400/20 flex items-center justify-center border border-amber-300/30 shrink-0 text-amber-300">
                          <ShieldCheck className="w-5 h-5" />
                        </div>
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            <h4 className="font-serif font-bold text-sm text-white">
                              Documento PDF de Políticas y Condiciones
                            </h4>
                            <span className="px-2 py-0.5 rounded-full bg-amber-400/20 text-amber-300 text-[10px] font-semibold border border-amber-400/30">
                              Visor Pop-up
                            </span>
                          </div>
                          <p className="text-[11px] text-sky-200/80 font-light mt-0.5 leading-relaxed">
                            Pega aquí el enlace de tu documento PDF (Google Drive, Dropbox o enlace web) o sube el archivo directamente. Cuando el cliente haga clic en <strong>«Ver Políticas y Condiciones»</strong>, este PDF se desplegará en un pop-up interactivo para lectura y descarga.
                          </p>
                        </div>
                      </div>

                      {/* Hidden file input for policy PDF upload */}
                      <input
                        type="file"
                        ref={policyPdfFileInputRef}
                        accept="application/pdf,.pdf"
                      ...
                      <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-xs space-y-3">
                        <div className="space-y-1.5">
                          <label className="block text-xs font-bold text-slate-800">
                            Enlace o URL del PDF de Políticas
                          </label>
                          <div className="flex flex-col sm:flex-row gap-2">
                            <div className="relative flex-1">
                              <input
                                type="url"
                                value={(editingDeparture as any).policyUrl || (editingDeparture as any).policy_url || ''}
                                onChange={(e) => {
                                  const val = e.target.value;
                                  setEditingDeparture({
                                    ...editingDeparture,
                                ...
                                placeholder="https://drive.google.com/file/d/.../view o https://tusitio.cl/politicas.pdf"
                                className="w-full text-xs text-slate-800 pl-9 pr-3 py-2.5 rounded-xl border border-slate-200 focus:border-[#0b192c] focus:outline-none bg-slate-50/50 focus:bg-white transition"
                              />
                            ...
                            <button
                              ...
                              onClick={() => policyPdfFileInputRef.current?.click()}
                              className="px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 font-semibold text-xs inline-flex items-center justify-center gap-1.5 transition cursor-pointer shrink-0 disabled:opacity-50"
                            >
                        ...
                        {/* Status preview / details if a URL is set */}
                        {Boolean((editingDeparture as any).policyUrl || (editingDeparture as any).policy_url) && (() => {
                          const currentPolicyUrl = (editingDeparture as any).policyUrl || (editingDeparture as any).policy_url || '';
                          const directUrl = getDirectPdfUrl(currentPolicyUrl);
                          const isGoogleDrive = currentPolicyUrl.includes('drive.google.com') || currentPolicyUrl.includes('docs.google.com');

                          return (
                            <div className="pt-3 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                              <div className="flex items-center gap-2.5 min-w-0">
                                <span className="px-2.5 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-800 text-[10px] font-bold inline-flex items-center gap-1 shrink-0">
                                  <CheckCircle2 className="w-3 h-3 text-emerald-600" />
                                  <span>PDF Conectado</span>
                                </span>
                                {isGoogleDrive && (
                                  <span className="px-2 py-0.5 rounded-full bg-sky-50 border border-sky-200 text-sky-800 text-[10px] font-medium shrink-0">
                                    Google Drive Adaptado
                                  </span>
                                )}
                                <span className="text-[11px] text-slate-500 font-mono truncate max-w-xs sm:max-w-sm">
                                  {currentPolicyUrl.startsWith('data:') ? 'Documento PDF codificado y listo' : currentPolicyUrl}
                                </span>
                              </div>
                              ...
                              <div className="flex items-center gap-2 shrink-0">
                                <a
                                  ...
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="px-3 py-1.5 rounded-xl bg-[#0b192c] hover:bg-slate-800 text-white font-semibold text-xs inline-flex items-center gap-1.5 transition cursor-pointer"
                                >
                                  <ExternalLink className="w-3.5 h-3.5 text-sky-300" />
                                  <span>Probar / Abrir</span>
                                </a>
                                <button
                                  type="button"
                                  onClick={() => {
                        ...
                        <p className="text-[10px] text-slate-400">
                          Soporta enlaces de Google Drive (compartidos como «Cualquier persona con el enlace»), Dropbox, enlaces web directos a archivos .pdf o archivos subidos desde tu equipo.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div>
                  {editDepartureTab === 'general' ? (
                    ...
                    <button
                      ...
                      className="text-xs text-sky-700 hover:text-sky-900 font-semibold flex items-center gap-1 cursor-pointer transition"
                    >
                      <span>Continuar a Brochure & Políticas</span>
                      <ChevronRight className="w-3.5 h-3.5" />
                    </button>
                  ) : (
                    <button
